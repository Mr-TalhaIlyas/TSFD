...
tumor_types = np.unique(np.load('./data/types.npy'))# give the list of tumour types
...
log_tb = TensorBoard(
    log_dir='./logs/{}/'.format(log_name))

callbacks = [LR_schedule, log_tb,
            ModelCheckpoint('./weights/{}.h5'.format(weight_name), verbose=1,
                            save_best_only=True, save_weights_only=True), # Save weights if val loss is improved
            CSVLogger('./logs/{}.csv'.format(log_name), separator=',', append=True)
            ]  
...
model.load_weights('./Efficent_pet_203_clf-end.h5')
